...
"""densityRegressor.py: quantum machine learning algorithm for regression of the probability density."""
...
class DensityRegression:
    def __init__(self, x, t, density, initial_conditions, N=3, bounds=[-2,2]):
    ...
    def objective(self, C):
        ...
        psi2 = self.qe.gaussianwavepacket(self.x, self.t, self.Q0)
        # Compute the RMSE loss
        loss = torch.sqrt(torch.mean((psi2 - self.Y) ** 2))
        # -----------------------------------------------------------------
        ...
        # Compute the penalty: not required if the ansatz is healthy
    ...
    def plot_predictions(self, x_test, t_test, Y_test, pdf_name=None):
        # Get optimal parameters
        self.qe.C = self.C_star
        # -----------------------------------------------------------------
        # Perform time evolution to get predictions for training set
        rho_train_pred = self.qe.gaussianwavepacket(self.x, self.t, self.Q0)
        rho_train_pred = rho_train_pred.detach().numpy()
        # -----------------------------------------------------------------
        # Convert torch tensors into numpy
        x_train = self.x.detach().numpy()
        t_train = self.t.detach().numpy()
        rho_train = self.Y.detach().numpy()
        # -----------------------------------------------------------------
        # Perform time evolution to get predictions for testing set
        rho_test_pred = self.qe.gaussianwavepacket(x_test, t_test, self.Q0)
        rho_test_pred = rho_test_pred.detach().numpy()
        # -----------------------------------------------------------------
        # Convert torch tensors into numpy
        x_test = x_test.detach().numpy()
        t_test = t_test.detach().numpy()
        rho_test = Y_test.detach().numpy()

        plots = PlotRegression()
        plots.get_plots(x_train, t_train, rho_train, rho_train_pred,
                        x_test, t_test, rho_test, rho_test_pred,
                        pdf_name=pdf_name)
...
class PlotRegression:
    ...
    def plot_density(self, x, t, y, score=None, clabel=r'$\rho$'):
        ...
        plt.tricontourf(x, t, y, levels=50, cmap='viridis')
        cbar = plt.colorbar(label=clabel)
         # Customise label font size and colour
        cbar.ax.set_ylabel(clabel, fontsize=self.fs, color=self.custom)   
        cbar.ax.tick_params(labelsize=self.fs, colors=self.custom)
        # Add annotations and format
        plt.xlabel(r'position', fontsize=self.fs, color=self.custom)
        plt.ylabel(r'time', fontsize=self.fs, color=self.custom)
        plt.xticks(fontsize=self.fs, color=self.custom)
        plt.yticks(fontsize=self.fs, color=self.custom)

        # Add score annotation if not None
        if score is not None:
            plt.annotate(f'$R^2$ = {score:.2f}',
                         xy=(0.95, 0.95), 
                         xycoords='axes fraction', 
                         fontsize=self.fs, color=self.custom, ha='right', va='top')

    def get_plots(self, 
                  x_train, t_train, Y_train, Y_train_pred, 
                  x_test, t_test, Y_test, Y_test_pred, 
                  pdf_name=None):
        ...
        print(f'Testing R2: {r2_test:.4f}, Testing RMSE: {rmse_test:.4f}')
        ...
        # Upper left subplot: True density with training data
        plt.subplot(2, 2, 1)
        self.plot_density(x_train, t_train, Y_train, clabel=r'$\hat{\rho}$')

        # Upper right subplot: Predicted density with training data
        plt.subplot(2, 2, 2)
        self.plot_density(x_train, t_train, Y_train_pred, score=r2_train, clabel=r'$\rho$')

        # Bottom left subplot: True density with testing data
        plt.subplot(2, 2, 3)
        self.plot_density(x_test, t_test, Y_test, clabel=r'$\hat{\rho}$')

        # Bottom right subplot: Predicted density with testing data
        plt.subplot(2, 2, 4)
        self.plot_density(x_test, t_test, Y_test_pred, score=r2_test, clabel=r'$\rho$')
...
class DataHandler:
    def __init__(self, filename):
        ...
        self.T, self.X, self.R = self.import_data()

    def import_data(self):
        # Read the data from CSV
        data = pd.read_csv(self.filename, header=None, names=['x', 't', 'r'])

        # Convert to torch tensors
        X = torch.tensor(data['x'].values, dtype=torch.float64)
        T = torch.tensor(data['t'].values, dtype=torch.float64)
        R = torch.tensor(data['r'].values, dtype=torch.float64)

        # Add 5% noise to the density R
        noise_R = 0.05 * R * torch.randn_like(R)
        R = R + noise_R
...
class TrainTestDataHandler(DataHandler):
    ...
    def get_train_data(self):
        return self.T[self.train_indices], self.X[self.train_indices], self.R[self.train_indices]

    def get_test_data(self):
        return self.T[self.test_indices], self.X[self.test_indices], self.R[self.test_indices]
